...
class MoonPhaseDma(BackTestBase):
    ...
    def generate_signals(self):
        df = self.dmgt.df
        df = MovingAverage(self.ma_1.get('period'), self.ma_1.get('source')).lin_reg(df, 'ma1')
        df = MovingAverage(self.ma_2.get('period'), self.ma_2.get('source')).vwma(df, 'ma2')
        ...
        df = RSI(self.rsi.get('length')).rsi(df, 'rsi')
...
if __name__ == '__main__':
    ma_1 = {'type': 'vwma' , 'period': 12, 'source': 'low'}
    ma_2 = {'type': 'lin_reg' , 'period': 14, 'source': 'low'}

    rsi_params = {'length': 24, 'long_max': 60, 'long_min': 29, 'short_max': 58, 'short_min': 51}
    chop_params = {'length': 4, 'maxci': 41, 'minci': 33}
    atr = {'length': 38}

    entry_exit_conds = {'risk':38, 'reward': 15}

    csv_path = r"C:\Users\user\MoonWalkerBacktester\Data\ETHUSD_Bybit15min.csv"
